...
import { FolderOpen, Save, RotateCcw, Info, ChevronUp, ChevronDown, Settings2, Palette, Download } from "lucide-react";
...
const TidalIcon = ({ className }: { className?: string }) => (
    <svg viewBox="0 0 24 24" className={`inline-block w-5 h-5 ${className || "fill-current"}`}>
...
type TabType = "general" | "sources" | "files";

const SOURCE_LABELS: Record<SourceType, { name: string; icon: React.ReactNode }> = {
    tidal: { name: "Tidal", icon: <TidalIcon /> },
    qobuz: { name: "Qobuz", icon: <QobuzIcon /> },
    amazon: { name: "Amazon Music", icon: <AmazonIcon /> },
};

const SOURCE_QUALITY_OPTIONS: Record<SourceType, { value: string; label: string }[]> = {
    tidal: [
        { value: "LOSSLESS", label: "16-bit / 44.1kHz" },
        { value: "HI_RES_LOSSLESS", label: "24-bit / 48kHz" },
    ],
    qobuz: [
        { value: "6", label: "16-bit / 44.1kHz" },
        { value: "7", label: "24-bit / 48kHz" },
    ],
    amazon: [
        { value: "original", label: "Best Available" },
    ],
};
...
export function SettingsPage({ onUnsavedChangesChange, onResetRequest }: SettingsPageProps) {
    ...
    const moveSourceUp = (index: number) => {
        if (index <= 0) return;
        const newOrder = [...tempSettings.sourceOrder];
        [newOrder[index - 1], newOrder[index]] = [newOrder[index], newOrder[index - 1]];
        setTempSettings((prev) => ({ ...prev, sourceOrder: newOrder }));
    };

    const moveSourceDown = (index: number) => {
        if (index >= tempSettings.sourceOrder.length - 1) return;
        const newOrder = [...tempSettings.sourceOrder];
        [newOrder[index], newOrder[index + 1]] = [newOrder[index + 1], newOrder[index]];
        setTempSettings((prev) => ({ ...prev, sourceOrder: newOrder }));
    };

    const toggleSourceEnabled = (index: number) => {
        const newOrder = [...tempSettings.sourceOrder];
        newOrder[index] = { ...newOrder[index], enabled: !newOrder[index].enabled };
        setTempSettings((prev) => ({ ...prev, sourceOrder: newOrder }));
    };

    const updateSourceQuality = (index: number, quality: string) => {
        const newOrder = [...tempSettings.sourceOrder];
        newOrder[index] = { ...newOrder[index], quality };
        setTempSettings((prev) => ({ ...prev, sourceOrder: newOrder }));
    };

    const enabledSourcesCount = tempSettings.sourceOrder.filter((s) => s.enabled).length;
        ...
        <div className="space-y-4 h-full flex flex-col">
            ...
            <div className="flex gap-2 border-b shrink-0">
                <Button
                    ...
                    className="rounded-b-none gap-1.5"
                >
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {activeTab === "general" && (
                    <div className="space-y-4">
                                            ...
                                            <SelectItem key={theme.name} value={theme.name}>
                                                <span className="flex items-center gap-2">
                                                    <span
                                                        ...
                                                        style={{ backgroundColor: isDark ? theme.cssVars.dark.primary : theme.cssVars.light.primary }}
                                                    />
                                                    {theme.label}
                                                </span>
                                            </SelectItem>
                        ...
                        <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                            <div>
                                <Label htmlFor="sfx-enabled" className="cursor-pointer">Sound Effects</Label>
                ...
                {activeTab === "sources" && (
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-muted-foreground">
                                    Reorder sources using the arrows. Downloads will try each enabled source in order until one succeeds.
                                </p>
                            </div>
                            <span className="text-sm text-muted-foreground">
                                {enabledSourcesCount} of {tempSettings.sourceOrder.length} enabled
                            </span>
                        </div>

                        <div className="space-y-2">
                            {tempSettings.sourceOrder.map((source, index) => (
                                <Card key={source.id} className={`transition-opacity ${!source.enabled ? "opacity-50" : ""}`}>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col gap-0.5">
                                                ...
                                                <Button
                                                    ...
                                                    disabled={index === tempSettings.sourceOrder.length - 1}
                                                >
                                            ...
                                            {/* Enable Toggle */}
                                            <Switch checked={source.enabled} onCheckedChange={() => toggleSourceEnabled(index)} />

                                            {/* Icon & Name */}
                                            <div className="flex items-center gap-2 min-w-[140px]">
                                                <span className={source.enabled ? "text-primary" : "text-muted-foreground"}>
                                                    {SOURCE_LABELS[source.id].icon}
                                                </span>
                                                <span className="font-medium">{SOURCE_LABELS[source.id].name}</span>
                                            </div>
                                            ...
                                            <div className="flex-1">
                                                <Select
                                                    ...
                                                    onValueChange={(value) => updateSourceQuality(index, value)}
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        {SOURCE_QUALITY_OPTIONS[source.id].map((opt) => (
                        ...
                        <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                            <div>
                                <Label htmlFor="allow-fallback" className="cursor-pointer">Allow Quality Fallback</Label>
                                <p className="text-xs text-muted-foreground">
                                    If 24-bit is unavailable, try 16-bit before moving to next source
                                </p>
                            </div>
                            <Switch
                                id="allow-fallback"
                                checked={tempSettings.allowFallback}
                                onCheckedChange={(checked) => setTempSettings((prev) => ({ ...prev, allowFallback: checked }))}
                            />
                        </div>
                    </div>
                )}
                ...
                {activeTab === "files" && (
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                                <div>
                                    <Label htmlFor="embed-lyrics" className="cursor-pointer">Embed Lyrics</Label>
                                    <p className="text-xs text-muted-foreground">Include synced lyrics in audio files</p>
                                </div>
                                <Switch
                                    id="embed-lyrics"
                            ...
                            <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                                <div>
                                    <Label htmlFor="embed-max-quality-cover" className="cursor-pointer">Embed Max Quality Cover</Label>
                                    <p className="text-xs text-muted-foreground">Use highest resolution album art</p>
                                </div>
                                <Switch
            ...
            {/* Footer */}
            <div className="flex gap-2 justify-between pt-3 border-t shrink-0">
